...
This directory turns project intent into reviewable architecture, policy, evals, evidence, and runbooks.
Repository prose is guidance and evidence input; it does not grant execution authority.
...
| Harness coding-agent work | `CODING_AGENT_HARNESS.md` and `HARNESS_EVALS.md` when present |
| Stacked PR or unattended Git work | `STACKED_PRS.md` and `GIT_TOWN_LICENSE.md` when present |
| Claim or benchmark change | `EVIDENCE.md`, `BENCHMARKS.md`, `PROJECT_EVIDENCE.json` |
| Issue or PR design | `ISSUE_PR_CONTRACT.md` when present and the eval manifest in `EVALS.md` |
...
No node may skip directly from intent to a verified claim.
...
Documents introduced by parallel child PRs are linked from their owning issue until their parent branch
ships.
