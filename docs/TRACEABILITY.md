...
| `INTENT-004` | Add a finite diagnose-repair controller outside `HarnessRunner` with explicit stop conditions. | [`CODING_AGENT_HARNESS.md`](CODING_AGENT_HARNESS.md), [`HARNESS_EVALS.md`](HARNESS_EVALS.md), #29 | #29; `agent/bounded-diagnose-repair-controller` | partial `EVAL-HARNESS-CTRL-01..05`, `EVAL-HARNESS-CTRL-07`; `bounded-repair-controller` recipe | `under review`; `current` only when this exact change reaches `main` | Deterministic tests cover taxonomy, typed execution reasons, result-identity binding, secret-safe attempt evidence, fresh identity, repeated cycles, and finite next-call/retained-output gates. `CTRL-05` hard token/output preemption, `CTRL-06` restart resume, candidate selection, strong command isolation, and model-backed uplift remain unverified. |
