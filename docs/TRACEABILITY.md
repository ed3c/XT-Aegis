...
| `INTENT-003` | Limit model/provider output to a bounded proposal; trusted code constructs the execution envelope. | [`CODING_AGENT_HARNESS.md`](CODING_AGENT_HARNESS.md), ADR 0005, #26 | #26; PR #51 `agent/provider-neutral-proposal-adapter` | `EVAL-HARNESS-PROP-01..06`, `trusted-proposal-envelope` recipe | `under review` in PR #51; `current` when this exact change is on `main` | Local fake-provider and adapter evals cover override, path, bytes, malformed/refused/timeout/truncation/oversize, model substitution, proxy/redirect, and fresh identity. No live Ollama correctness, availability, privacy, or version-attestation evidence. |
