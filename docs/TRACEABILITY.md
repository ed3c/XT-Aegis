...
|---|---|---|---|---|---|---|
| `INTENT-001` | Bind approvals and idempotency to a versioned canonical request and policy identity. | [`CODING_AGENT_HARNESS.md`](CODING_AGENT_HARNESS.md), [`HARNESS_EVALS.md`](HARNESS_EVALS.md), #24 | #25; PR #31 `agent/harness-request-identity-exit-contract` | `EVAL-HARNESS-ID-01..06`, #25 substitution/replay cases | `under review` in PR #31; `current` when this change is on `main` | Full local gates and two-axis review passed; GitHub required checks and merge authority remain the acceptance gate. |
| `INTENT-002` | Honor declared `expected_exit_codes` for command actions while preserving assertion and rollback semantics. | [`CODING_AGENT_HARNESS.md`](CODING_AGENT_HARNESS.md), #28 | #28; PR #31 | `EVAL-HARNESS-EXEC-01..03`, #28 validation/result/event cases | `under review` in PR #31; `current` when this change is on `main` | Full local gates and two-axis review passed; GitHub required checks and merge authority remain the acceptance gate. |
...
| PR #31 `agent/harness-request-identity-exit-contract` | `under review` when this table is read from PR #31; `current` when this exact change is on `main` | Before merge, publish evidence metadata and require GitHub checks plus merge authority; once on `main`, no delivery action remains for #25/#28. |
