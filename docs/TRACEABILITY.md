...
| `INTENT-004` | Add a finite diagnose-repair controller outside `HarnessRunner` with explicit stop conditions. | [`CODING_AGENT_HARNESS.md`](CODING_AGENT_HARNESS.md), [`HARNESS_EVALS.md`](HARNESS_EVALS.md), #29 | #29; `agent/bounded-diagnose-repair-controller` | `EVAL-HARNESS-CTRL-01..07`, `bounded-repair-controller` recipe | `under review`; `current` only when this exact change reaches `main` | Deterministic tests cover taxonomy, fresh identity, bounded redacted diagnostics, repeated cycles, and finite budgets. Process-restart resume, candidate selection, strong command isolation, and model-backed uplift remain unverified. |
