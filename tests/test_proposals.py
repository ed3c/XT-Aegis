...
def test_valid_proposal_builds_trusted_action_envelope(compiled_skill) -> None:  # type: ignore[no-untyped-def]
    ...
    assert envelope.provider_profile == profile
    ...
    assert envelope.request.action.content == outcome.proposal.content
...
def test_fake_provider_returns_deterministic_non_execution_results(
...
def test_non_ready_outcome_cannot_build_an_action_request(compiled_skill) -> None:  # type: ignore[no-untyped-def]
    profile = ProviderProfile(
        provider="fake",
        model="deterministic",
        version="1.0",
    ...
    with pytest.raises(ValueError, match="ready proposal outcome"):
        build_action_request(
            ProposalOutcome(status=ProposalStatus.MALFORMED, profile=profile),
            trusted=TrustedEnvelopeConfig(target_path="sample_project/app.py"),
            skill=compiled_skill,
        )
